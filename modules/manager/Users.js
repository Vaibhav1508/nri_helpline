...
let Login = async (body) => {
    ...
    let findData = {}
    findData["$or"] = [
        { userEmail: { $eq: body.userEmail } },
    ]
    findData["$and"] = [
        { userPassword: { $eq: md5(body.userPassword) } }
    ]
    ...
    if(user.userStatus == 'Active') {
        ...
        await UserModel.update({userSecurityToken : authToken},{ where: {userID : user.userID}, raw: true });
        let industry = await IndustryModel
        .findOne({ where: {industryID : user.industryID}, raw: true });
...
let register = async (body) => {
    ...
    ['userFirstName', 'userLastName','userEmail', 'userPassword', 'userMobile', 'industryID','userJobDescription', 'userType' ].forEach(x => {
        if (!body[x]) {
            throw new BadRequestError(x + " is required");
        }
    });
...
let ResendOtp = async (body) => {
    ...
    let otp = Math.floor(100000 + Math.random() * 900000)
    await UserModel.update({userOTP:otp,userVerified:'No'},{where: {userID: user.userID }, raw: true});
    return {otp : otp};
}
...
let ForgetPassword = async (body) => {
    ...
    let otp = Math.floor(100000 + Math.random() * 900000)
    await UserModel
        .update( {userOTP : otp},{ where: { userEmail: body.userEmail.trim()} , raw: true });

    
    return {messge : "OTP has been sent successfully to registerd email", otp : otp};
}
...
let SetNewPassword = async (req) => {
    ...
    let user = await UserModel
        .findOne({ where: { userEmail: body.userEmail.trim(), userSecurityToken : req.headers.token} , raw: true });
        if (!user) {
            //error
            throw new BadRequestError("User does not exist")
        }
    let authToken = await generateAuthToken(body.userEmail);
    await UserModel.update({userPassword : md5(body.userPassword.trim()), userSecurityToken:authToken,userOTP:''}, { where: { userEmail: body.userEmail.trim()} , raw: true });
...
let ProfileSetup = async (body) => {
    ...
    await UserModel.update({userOtherDetails : body?.userOtherDetails.trim(), userProfileSetupStatus : 'Yes'}, { where: { userID: body.userID} , raw: true });
...
let UserUpdate = async (req) => {
    ...
    ['userFirstName', 'userLastName', 'industryID','userJobDescription' ].forEach(x => {
        if (!body[x]) {
            throw new BadRequestError(x + " is required");
        }
    });
    let updateData = {};
    let optionalFiled = ['userFirstName', 'userLastName', 'industryID','userJobDescription'];
    optionalFiled.forEach(x => {
        if (body[x]) {
            updateData[x] = body[x];
        }
    ...
    await UserModel.update(updateData, { where: { userID: req.params.userID }, raw: true });
    let  user = await UserModel.findOne({where:{userID: req.params.userID},raw:true});
    let industry = await IndustryModel
        .findOne({ where: {industryID : user.industryID}, raw: true });
    user.industryName = industry.industryName
    user.userProfilePicture = config.upload_folder + config.upload_entities.user_profile_image_folder + user.userProfilePicture;
