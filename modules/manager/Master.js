...
let createCountry = async (req) => {
    ...
    ['countryName','countryCode','countryRemarks','countryShortName','countryCurrency'].forEach(x => {
        if (!body[x]) {
    ...
    let country = await CountryModel
        .findOne({ where: {countryName: body.countryName}, raw: true });
    ...
    let countryData = {
        ...
        countryRemarks: body.countryRemarks,
        countryStatus: countryStatus,
        countryShortName: body.countryShortName,
        countryCurrency : body.countryCurrency
    }

    let countryDetail = await VocationModel.create(countryData);
    countryDetail.countryFlagImage = config.upload_folder + config.upload_entities.country_image_folder + countryDetail.countryFlagImage;
...
let CountryUpdate = async (req) => {
    ...
    let country = await CountryModel
        .findOne({ where: {countryID: req.params.countryID}, raw: true });
    ...
    let updateData = {};
    let optionalFiled = ['counryID', 'countryName','countryRemarks','vocationStatus','countryShortName','countryCurrency'];
    ...
    if (req.file && req.file.path) {
        updateData["countryImage"] = req.file.filename;
    }
    await CountryModel.update(updateData, { where: { countryID: req.params.countryID }, raw: true });
    let  countryData = await CountryModel.findOne({where:{countryID: req.params.countryID},raw:true});
    countryData.countryFlagImage = config.upload_folder + config.upload_entities.country_image_folder + countryData.countryFlagImage;
    ...
    if(body.page || body.limit) {
       let allCountry = await countryModel.findAll({
        ...
        for(let i=0 ; i<allCountry.length; i++) {
            allCountry[i].countryFlagImage = config.upload_folder + config.upload_entities.country_image_folder + allCountry[i].countryFlagImage;
        }
